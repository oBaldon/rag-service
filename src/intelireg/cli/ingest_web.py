...
def extract_nodes_by_headings(html: str, max_heading_level: int = 3) -> tuple[str, List[NodeDraft], str]:
...
# --------- DB: upsert simplificado por URL / dedup por content_hash ---------
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Ingestão MVP: URL -> nodes (headings) -> READY_FOR_INDEX + enqueue IndexVersionJob")
    ap.add_argument("--url", required=True)
    ap.add_argument("--source-org", required=True)
    ap.add_argument("--doc-type", required=True)
    ap.add_argument("--max-heading-level", type=int, default=3)

    # defaults alinhados ao canvas
    ap.add_argument("--pipeline-version", default=None)
    ap.add_argument("--embedding-model-id", default=None)

    args = ap.parse_args()

    pipeline_version = args.pipeline_version or "mvp-v1"
    embedding_model_id = args.embedding_model_id or "text-embedding-3-small@1536"

    # 1) fetch HTML (apenas na ingestão)
    with httpx.Client(follow_redirects=True, timeout=30.0, headers={"User-Agent": "InteliReg-MVP/0.1"}) as client:
    ...
    if http_status < 200 or http_status >= 300:
        raise SystemExit(f"HTTP {http_status} ao buscar {args.url} (final_url={final_url})")

    html = resp.text

    # 2) canonicalizar em nodes por headings e gerar content_hash
    title, node_drafts, content_hash = extract_nodes_by_headings(html, max_heading_level=args.max_heading_level)

    # 3) gravar no banco (snapshot derivado = nodes)
    document_id = None
    ...
    for i, nd in enumerate(node_drafts):
        ...
        node_rows.append((node_id, version_id, i, nd.path, parent_id, nd.heading_text, nd.heading_level, nd.text_normalized))
    ...
    with get_conn() as conn:
        with conn.cursor() as cur:
            ...
            cur.execute(
                ...
                """,
                (version_id, document_id, args.url, final_url, http_status, captured_at, content_hash),
            )
    ...
    print(f"[ingest_web] ok version_id={version_id} document_id={document_id} nodes={len(node_rows)} content_hash={content_hash}")
